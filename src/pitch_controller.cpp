...
constexpr float kBallPossessionRangeThresholdCm = 80.0f;
constexpr float kBallDistanceFromOwnerCm = 41.0f;

}  // namespace

PitchController::PitchController(Pitch& pitch)
    : pitch_(pitch),
      player_controlled_by_user_(0) {}

void PitchController::ApplyUserInput(const std::set<UserInput>& user_input) {
  //   std::cout << __func__ << "() player: " << player_controlled_by_user_ <<
  //   "\n";
  auto& player = pitch_.players.at(player_controlled_by_user_);

  const auto previous_player_position = player.position;
    ...
    switch (action) {
      ...
      case UserInput::Kick:
        ...
        const auto player_ball_relation =
            pitch_.ball.position - player.position;
        ...
        if (distance_to_ball <= 150.0f) {
          ...
          const auto kick_direction = player.direction;
  ...
  //   std::cout << __func__ << "() move_decision: "
  //             << move_decision.Normalize() * kPlayerMoveValueCm << "\n";
  player.position += move_decision.Normalize() * kPlayerMoveValueCm;

  const auto position_change =
      physics2d::Vector<float>{player.position - previous_player_position}
          .Normalize();
  // TODO: don't compare floats directly
  // Prevent setting direction to zero.
  if (position_change.x != 0.0f || position_change.y != 0.0f) {
    player.direction = position_change;
  }
}

void PitchController::ApplyAgentDecisions(
    const std::map<unsigned, AgentDecision>& decisions) {
  for (const auto& [player_id, decision] : decisions) {
    // TODO
    std::cout << __func__ << "() player: " << player_id << "\n";
    auto& player = pitch_.players.at(player_id);
    ...
    std::cout << __func__
              << "() move_decision: " << decision.player_move.Normalize()
              << "\n";
    player.position += decision.player_move.Normalize() * kPlayerMoveValueCm;

    const auto position_change =
        physics2d::Vector<float>{player.position - previous_position}
            .Normalize();
    // TODO: don't compare floats directly
    // Prevent setting direction to zero.
    if (position_change.x != 0.0f || position_change.y != 0.0f) {
  ...
  //   std::cout << __func__
  //             << "() decisions.player_move: " <<
  //             decisions.player_move.Normalize()
  //             << "\n";
  //   pitch_.bot.position += decisions.player_move.Normalize() *
  //   kPlayerMoveValueCm;
...
void PitchController::UpdatePitch() {
  ...
  // TODO: provide all agents with the same version of the pitch

  for (const auto& [id, player] : pitch_.players) {
    const auto player_ball_relation = pitch_.ball.position - player.position;
    if (player_ball_relation.GetLength() <= kBallPossessionRangeThresholdCm) {
      pitch_.ball.velocity = {0, 0};
      pitch_.ball.position =
          player.position + player.direction * kBallDistanceFromOwnerCm;
    }
  }
